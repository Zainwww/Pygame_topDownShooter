...
HUD_H = 70
SCREEN_W, SCREEN_H = MAP_W, MAP_H + HUD_H
FPS = 60
...
GHOST_SPRITE_PATHS = {
    "Ghost": os.path.join(SPRITE_DIR, "normal.png"),
    "FastGhost": os.path.join(SPRITE_DIR, "speed.png"),
    "DasherGhost": os.path.join(SPRITE_DIR, "dash.png"),
    "PoisonGhost": os.path.join(SPRITE_DIR, "poison.png"), 
}
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Nightmare Corridor")
        ...
        self.gacha_banner = None   # {"text","color","icon","timer"} saat peti dibuka
    ...
    def update(self, dt):
        ...
        # buka peti = gacha (dapat 1 jenis senjata acak sesuai rarity)
        for chest in self.chests[:]:
            chest.update(dt)
            if chest.get_rect().colliderect(self.player.get_rect()):
    ...
    def draw_hud(self, surf):
        pygame.draw.rect(surf, HUD_BG, (0, MAP_H, SCREEN_W, HUD_H))
        bar_w = 220
        pygame.draw.rect(surf, (60, 15, 15), (16, MAP_H + 18, bar_w, 22))
        ratio = self.player.hp / self.player.max_hp
        pygame.draw.rect(surf, GREEN if ratio > 0.3 else RED, (16, MAP_H + 18, bar_w * ratio, 22))
        pygame.draw.rect(surf, WHITE, (16, MAP_H + 18, bar_w, 22), 2)
        surf.blit(self.font_small.render(f"HP {int(self.player.hp)}/{self.player.max_hp}", True, WHITE),
                   (16 + bar_w + 10, MAP_H + 20))
        if self.player.poison_time > 0:
            surf.blit(self.font_small.render("☠ Teracun!", True, (140, 220, 110)), (16, MAP_H + 2))

        weapon = self.player.weapon
        icon_x = 16
        icon = self.weapon_icons.get(type(weapon).__name__)
        if icon:
            surf.blit(icon, (icon_x, MAP_H + 38))
            text_x = icon_x + icon.get_width() + 8
        else:
            text_x = icon_x
        empty = weapon.ammo <= 0
        blink_on = (pygame.time.get_ticks() // 200) % 2 == 0
        ammo_color = RED if (empty and blink_on) else (RED if empty else YELLOW)
        surf.blit(self.font.render(f"{weapon.name}", True, YELLOW), (text_x, MAP_H + 42))
        name_w = self.font.size(weapon.name)[0]
        ammo_label = "KOSONG!" if empty else f"{weapon.ammo}/{weapon.max_ammo}"
        surf.blit(self.font.render(f"  {ammo_label}", True, ammo_color), (text_x + name_w, MAP_H + 42))

        surf.blit(self.font.render(f"Skor: {self.score}", True, WHITE), (SCREEN_W - 220, MAP_H + 18))
        surf.blit(self.font_small.render(f"Hantu dibunuh: {self.kills}", True, (180, 180, 190)),
                   (SCREEN_W - 220, MAP_H + 42))
    ...
    def draw_hit_flash(self, surf):
        if self.player.hit_flash > 0:
            ...
            flash = pygame.Surface((SCREEN_W, MAP_H), pygame.SRCALPHA)
            flash.fill((200, 0, 0, alpha))
            surf.blit(flash, (0, 0))

    def draw_menu(self):
        self.screen.fill(BLACK)
        title = self.font_big.render("NIGHTMARE CORRIDOR", True, RED)
        self.screen.blit(title, title.get_rect(center=(SCREEN_W / 2, SCREEN_H / 2 - 90)))
        lines = [
            "Kamu terjebak dalam mimpi buruk... tapi kamu menyukainya.",
            ...
            "Tidak ada batas waktu -- bertahan hiduplah selama mungkin",
            "",
            "Tekan SPACE untuk mulai",
        ]
        for i, line in enumerate(lines):
            txt = self.font.render(line, True, WHITE)
            self.screen.blit(txt, txt.get_rect(center=(SCREEN_W / 2, SCREEN_H / 2 - 10 + i * 30)))
        pygame.display.flip()

    def draw_gameover(self):
        overlay = pygame.Surface((SCREEN_W, MAP_H), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 190))
        self.screen.blit(overlay, (0, 0))
        title = self.font_big.render("KAMU TERTANGKAP", True, RED)
        self.screen.blit(title, title.get_rect(center=(SCREEN_W / 2, MAP_H / 2 - 50)))
        sc = self.font.render(f"Skor akhir: {self.score}   |   Hantu dibunuh: {self.kills}", True, WHITE)
        self.screen.blit(sc, sc.get_rect(center=(SCREEN_W / 2, MAP_H / 2)))
        hint = self.font_small.render("Tekan R untuk main lagi, ESC untuk keluar", True, (200, 200, 200))
        self.screen.blit(hint, hint.get_rect(center=(SCREEN_W / 2, MAP_H / 2 + 36)))
    ...
    def draw(self):
        ...
        self.draw_hit_flash(self.screen)
        self.draw_gacha_banner(self.screen)
